...
def detect_s4t_mutations(pairsam_path, output, chunksize):
...
def fill_n_s4t_pairs(in_path, out_path, chunksize=int(1e6)):
    ...
    new_header = pairtools._headerops.extract_fields(header, "columns", save_rest=True)[
        1
    ]
    ...
    chunk_iterator = pd.read_csv(
        pairs_body, header=None, names=cols, chunksize=chunksize, sep="\t"
    )
    ...
    for chunk_df in chunk_iterator:
        ...
        chunk_df.to_csv(
            out_stream, sep="\t", columns=new_cols, header=False, index=False,
        )
...
def fill_n_s4t_df(pairs_df, samheader):
    ...
    for i in pairs_df.index:
        for side in ["1", "2"]:
            mms, n_matches = get_mismatches_samtxt(
                pairs_df.at[i, "sam" + side], samheader=samheader
            )
            n_AG_muts_phred0 = 0
            n_AG_muts_phred30 = 0
            n_TC_muts_phred0 = 0
...
def get_mismatches_samtxt(sam_col, samheader):
    sam_strs = [
        sam_str.replace(pairtools._pairsam_format.SAM_SEP, "\t")
        for sam_str in sam_col.split(pairtools._pairsam_format.INTER_SAM_SEP)
    ]
    sam_algns = [
        pysam.AlignedSegment.fromstring(sam_str, samheader) for sam_str in sam_strs
    ]
    algn = sorted(
        sam_algns,
        key=lambda _: (
            _.query_length - _.query_alignment_end
            if _.is_reverse
            else _.query_alignment_start
        ),
    )[0]
